...
class UI:
    ...
    def show_isim(self,isim):
        txt_sur = self.font.render(str(isim),False ,txt_color)
 
        x = self.display_surface.get_size()[0] - 40
        y = self.display_surface.get_size()[1] - 630
        txt_rect = txt_sur.get_rect(bottomright=(x, y))

        pygame.draw.rect(self.display_surface, ui_bg_color, txt_rect.inflate(20, 20))
        self.display_surface.blit(txt_sur, txt_rect)
        pygame.draw.rect(self.display_surface, ui_border_color, txt_rect.inflate(10, 10), 3)
